...
from similarity_app.config import EMBEDDINGS_CACHE_PATH, SIMILARITY_THRESHOLD, TOP_K_SIMILAR, VERBOSE
from similarity_app.services.embedding_service import EmbeddingService
from similarity_app.services.llm_service import LLMService
from similarity_app.similarity_matcher import SimilarityMatcher
...
class SimilarityMatchingApp:
    ...
    def __init__(self) -> None:
        ...
        self.base_embeddings: Optional[Dict[str, np.ndarray]] = None
        self.base_texts: Optional[List[str]] = None
        self.similarity_method: str = AVAILABLE_SIMILARITY_METHODS["余弦相似度"]

    def load_base_embeddings(self) -> bool:
        """加载基础题库的向量数据"""
        if self.base_embeddings is not None and self.base_texts is not None:
            return True
        ...
        try:
            ...
            if cache_path.exists():
                ...
                if isinstance(cache_data, dict):
                    ...
                    if "embeddings" in cache_data and isinstance(cache_data["embeddings"], dict):
                ...
                st.error("❌ 缓存文件格式不正确")
                return False
            st.error(f"❌ 未找到基础题库向量文件: {cache_path}")
            return False
        except Exception as exc:  # pylint: disable=broad-except
            st.error(f"❌ 加载基础题库向量数据失败: {str(exc)}")
            return False
    ...
    async def process_user_texts(self, user_texts: List[str]) -> List[np.ndarray]:
        ...
        try:
            ...
            st.error(f"向量化处理失败: {str(exc)}")
            return []

    def find_similar_texts(
        self, user_embeddings: List[np.ndarray], user_texts: List[str]
    ) -> Dict[str, List[Tuple[str, float]]]:
        """查找相似文本"""
        similarity_results: Dict[str, List[Tuple[str, float]]] = {}

        for user_embedding, user_text in zip(user_embeddings, user_texts):
            try:
                similar_items = self.similarity_matcher.find_most_similar(
                    query_embedding=user_embedding,
                    candidate_embeddings=self.base_embeddings or {},
                    similarity_method=self.similarity_method,
                )
                similarity_results[user_text] = similar_items
            except Exception as exc:  # pylint: disable=broad-except
                st.error(f"处理文本 '{user_text[:50]}...' 时发生错误: {str(exc)}")
    ...
    async def enhance_with_llm(
        ...
        """使用LLM增强相似度结果"""
        try:
            return await self.llm_service.enhance_similarity_results(similarity_results)
        except Exception as exc:  # pylint: disable=broad-except
            st.error(f"LLM增强处理失败: {str(exc)}")
    ...
    def create_output_dataframe(self, enhanced_results: Dict[str, List[Tuple[str, float, str]]]) -> pd.DataFrame:
        ...
        for query_text, results in enhanced_results.items():
            row_data: Dict[str, Any] = {OUTPUT_COLUMNS["query"]: query_text}
...
def main() -> None:
    ...
    app: SimilarityMatchingApp = st.session_state.app

    with st.sidebar:
        st.header("⚙️ 系统配置")

        app.similarity_method = AVAILABLE_SIMILARITY_METHODS[
            st.selectbox("相似度计算方式", list(AVAILABLE_SIMILARITY_METHODS.keys()))
        ]
        app.similarity_matcher.top_k = st.slider("Top-K 返回数量", 1, 5, TOP_K_SIMILAR)
        app.similarity_matcher.threshold = st.slider(
            "相似度阈值",
            0.0,
            1.0,
        ...
        use_llm = st.checkbox("启用LLM语义增强", value=True,
                              help="关闭后仅使用向量相似度，降低时延和API开销")

        st.markdown("---")
        st.subheader("📊 系统状态")

        if app.load_base_embeddings():
            st.success(f"✅ 基础题库已加载 ({len(app.base_texts or [])} 条)")
            base_loaded = True
        else:
            st.error("❌ 基础题库加载失败")
            base_loaded = False

        st.caption(f"向量缓存路径: {Path(EMBEDDINGS_CACHE_PATH)}")

    if not base_loaded:
        st.error("⚠️ 系统初始化失败，请检查基础题库文件是否存在")
        st.info(f"请确保文件存在: {Path(EMBEDDINGS_CACHE_PATH)}")
        return
    ...
    if uploaded_file is not None:
        try:
            ...
            with st.expander("📋 数据预览", expanded=False):
                preview_df = pd.DataFrame(
                    {"序号": range(1, min(11, len(user_texts) + 1)), "题目文本": user_texts[:10]}
                )
            ...
            with col2:
                if st.button("🚀 开始匹配", type="primary", use_container_width=True):
                    ...
                    try:
                        ...
                        status_text.text("🤖 正在使用LLM增强结果..." if use_llm else "📊 跳过LLM，仅使用向量结果")
                        ...
                        if use_llm:
                            ...
                            enhanced_results = {
                                query_text: [(text, score, "未使用LLM增强") for text, score in candidates]
                                for query_text, candidates in similarity_results.items()
                            }
                        ...
                        with col4:
                            ...
                            for i in range(1, 4):
                                ...
                                high_quality_matches += len(
                                    result_df[(result_df[score_col] != "") & (result_df[score_col] >= 0.7)]
                                )
        ...
        with st.expander("📖 使用说明", expanded=True):
            ...
            ### ⚙️ 系统特性
             ...
             - **多种相似度算法**: 余弦、欧几里得、点积可选
             - **智能匹配**: 结合向量相似度和可选的LLM语义增强
             - **高效处理**: 支持批量并发处理
